...
> **✅ УЖЕ НАСТРОЕНО (2026-07-05).** Проект `gavblurlfvkabolwpmoi` (Duushes's Project, Frankfurt):
> таблица `progress` + RLS созданы, автоподтверждение включено (вход без письма), ключи в GitHub
> secrets и `.env.local`. UX — логин/пароль: логин превращается в `login@duushes.github.io` под
> капотом (домен обязан резолвиться — GoTrue проверяет DNS). Инструкция ниже — на случай пересоздания.
